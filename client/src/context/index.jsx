...
export const StateContextProvider = ({ children }) => {
  const { contract } = useContract(
    "0xC931DBe6248B7A04de11f1fFBfCbe7bfc4043223"
  );
    ...
    try {
      const data = await createCampaign({
        args: [
          title,
          description,
          ethers.utils.parseEther(target.toString()), // 確保目標金額是以ETH為單位，並轉換為Wei
          Math.round(new Date(deadline).getTime() / 1000), // 將毫秒時間戳轉換為秒時間戳
          image,
        ],
      });
  ...
  const fetchNumberOfCampaigns = async () => {
    console.log("區塊鏈動作，抓取numberOfCampaigns");
    if (!contract) {
      throw new Error("Contract is not initialized");
    }
